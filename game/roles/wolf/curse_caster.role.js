...
exports.abilities.curse = {
  ...
  run(game, me, args, cb) {
    game.masters.tell(me.id, `attempting to curse ${args[0]}`)
    game.u.resolve_to_id(args[0]).then(t=>{
      p = game.player(t)
      if(p.role == "inno/cursed") {
        cb(true, `Your curse result is **neutral**. You haven't cursed ${p}, because they already were a **Cursed Civilian**!`)
        if(p.role == "inno/basic") {
          p.role = "inno/cursed"
          p.tell("**You have been cursed!** The curse caster has chosen you, an innocent, to get cursed!\nThis means that if the werewolves attack you, you will now become a werewolf yourself! (Instead of dying)")
          cb(true, `Your curse result is **positive**. You have cursed ${p} successfully, and they are now a **Cursed Civilian**!`)
        } else {
          me.tell()
